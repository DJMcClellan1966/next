"""Deep Learning Lab — Goodfellow, Bishop, ESL, Burkov. Run from repo root: python learning_apps/deep_learning_lab/app.py"""
import sys
...
for p in (str(REPO_ROOT), str(LAB_DIR)):
    if p not in sys.path:
        sys.path.insert(0, p)
APP_TITLE = "Deep Learning Lab"
APP_DESCRIPTION = "Goodfellow/Bengio/Courville, Bishop, ESL, Burkov. Learn and try demos."
try:
...
def api_health(): return jsonify({"ok": True, "curriculum": CURRICULUM_AVAILABLE, "demos": DEMOS_AVAILABLE})
...
def _html():
    ...
    r""".c{max-width:900px;margin:0 auto;}h1{font-size:1.5rem;}.sub{color:#94a3b8;margin-bottom:16px;}"""
    r""".tabs{display:flex;gap:8px;flex-wrap:wrap;margin:16px 0;}.tabs button{padding:10px 16px;border:1px solid #475569;border-radius:8px;background:#1e293b;color:#e2e8f0;cursor:pointer;}.tabs button.active{background:#3b82f6;}"""
    ...
    r"""<div id="book-topics" style="margin-top:12px;"></div><div id="topic-detail" style="margin-top:16px;display:none;"><h3 id="topic-title"></h3><p><strong>Learn</strong></p><p id="topic-learn" class="out" style="max-height:100px;"></p>"""
